...
function* getAccounts() {   //будет вызываться при монтировании основного компонента
    try {
        const accounts = yield call(getAccountsFirebase);
        yield put(getAccountsSuccess(accounts));  //будет вызываться при успешном получении данных
    } catch (err) {
        yield put(showNotification(NOTIFICATION_STATUS.ERROR, NOTIFICATION_MESSAGE.GET_ACCOUNTS_ERROR)); //будет вызываться при возникновении ошибки при получении данных
    }
}
